...
class Generator {
    ...
    //Output finish message, trigger build, and log status
    finish() {
        console.log('Generating a website for your team now :)');
    ...
    addIntern() {
        this.addEmployee().then(employee => {
            return inquirer
                ...
                .then(internData => {
                    // Add intern to team
                    this.team.push(new Intern(employee.name, employee.email, internData.github, employee.id));
    ...
    build() {
        ...
        const webPage = new Promise((resolve, reject) => {
            const buildErrors = [];
            ...
            if (buildErrors.length == 0) {
                resolve({
                    ok: true,
                    message: 'Team website created successfully!'
                });
            } else {
                reject(buildErrors.join('\n'));
            }
        });
    }
}
